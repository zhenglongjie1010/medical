<template>
    <div class="table">
        <div class="crumbs">
            <i class="el-icon-tickets">就诊人信息</i>
        </div>
        <div class="container">
            <div class="handlebox">
       ...
       <el-table size="mini" border style="width:100%" height="680px" :data="tableData" @selection-change="handleSelectionChange">
        ...
        <el-dialog title="删除就诊人" :visible.sync="delVisible" width="400px" center>
...
import {selectByPrimaryKey,setPatient,updatePatient,delPatient} from '../api/index';
...
export default {
    ...
    data(){
        ...
        return {
            ...
            doctorName: '',  //前端传过来的接诊医生名字
            doctorId: '' ,  // 前端传过来的接诊医生编号
        ...
        select_word: function(rucan){
            ...
            if(this.select_word == ''){
                ...
                for(let item of this.tempData){
                    if(item.doctorName.includes(this.select_word)){
                        //console.log(this.tableData);
    ...
    created(){
         ...
         this.select_word = this.doctorName;
         this.getData();
    },
    methods:{
        ...
        getData(){
            this.tempData = [];
            this.tableData= [];
            selectByPrimaryKey(this.doctorId).then(res => {
                this.tempData= res;
                this.tableData = res
                .map(item => {
                    if(item.doctorName.indexOf(this.doctorName)>-1){
                        return item
                    }
                })
               console.log(this.tableData)
        ...
        //更新图片
        uploadUrl(id){
            return `${this.$store.state.HOST}/patient/updatepatientPic?id=${id}`
        },
        ...
        addPatient(){
            ...
            params.append('doctorName',this.doctorName);
